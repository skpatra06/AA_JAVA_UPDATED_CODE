...
public class ReadContextualTeaser extends DecisionElementBase{
	...
	public String doDecision(String elementName, DecisionElementData data) throws Exception {
		...
		try {
			flagMap = (Map<String, Map<String, String>>) data.getApplicationAPI().getGlobalData("CT_HYBRID");
			util.addToLog(elementName+""+ flagMap,util.DEBUGLEVEL);
				...
				for(int i=1;i<=flagMap.size();i++) 
				{
					...
					if(util.IsNotNUllorEmpty(map)) 
					{
						...
						if("Y".equalsIgnoreCase(strFlag.trim()))
						{
							...
							{
							case 1:
								if(null!=responseValue && responseValue.equalsIgnoreCase(String.valueOf(map.get("Condition Value")).trim()))
								{
							...
							/*
							 * if("Hours".equalsIgnoreCase(""+map.get("Condition Type"))) { boolean
							 * expireTimeFlag=isValidExpireTimeHours(responseValue,""+map.
							 * get("Condition Type").trim(),"HH",util); if(expireTimeFlag) {
							 * 
							 * 
							 * } }else if("Day".equalsIgnoreCase(""+map.get("Condition Type"))){ boolean
							 * expireTimeFlag=isValidExpireTimeDays(responseValue,""+map.
							 * get("Condition Type").trim(),"dd",util); if(expireTimeFlag) {
							 * 
							 * } }else if("Currency".equalsIgnoreCase(""+map.get("Condition Type"))){
							 * boolean isAvailableBalance=isBalanceAvailable(responseValue,""+map.
							 * get("Condition Type").trim()); if(isAvailableBalance) {
							 * 
							 * }
							 */
						}
